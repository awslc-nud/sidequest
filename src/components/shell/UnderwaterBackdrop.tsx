 ...
 * scene, all built from `ui-spec.md` §1.2 `brand-*` tokens:
...
export default function UnderwaterBackdrop() {
  return (
    <div aria-hidden="true" className="pointer-events-none absolute inset-0 z-0 overflow-hidden">
      ...
      <div className="absolute -top-24 left-[7%] h-[64vh] w-16 -rotate-[15deg] bg-gradient-to-b from-brand-white/85 via-brand-white/35 to-transparent blur-2xl" />
      <div className="absolute -top-24 left-[40%] h-[78vh] w-28 -rotate-[10deg] bg-gradient-to-b from-brand-white/75 via-brand-white/25 to-transparent blur-3xl" />
      <div className="absolute -top-24 right-[8%] h-[60vh] w-20 -rotate-[20deg] bg-gradient-to-b from-brand-accent/60 via-brand-accent/20 to-transparent blur-2xl" />

      {/* Soft spotlight behind the chest. */}
      <div className="absolute left-1/2 top-24 h-64 w-64 -translate-x-1/2 rounded-full bg-brand-accent/20 blur-3xl" />
      <div className="absolute left-1/2 top-28 h-40 w-40 -translate-x-1/2 rounded-full bg-brand-white/55 blur-2xl" />
    </div>
  );
}
