...
| 31 | Button / color hierarchy | Unified interactive colors into four tiers: **primary/selected** `bg-brand-deep text-brand-white` hover `bg-brand-ink`; **secondary/cancel** `border-brand-track text-brand-deep` hover `bg-brand-track/40`; **disabled** `border-brand-track bg-brand-track/30 text-brand-muted` (solid, no dashed); **ghost** `text-brand-muted` hover `bg-brand-bg`. The survey CTA lost its one-off `bg-brand-accent` fill — `brand-accent` is now reserved for progress fill, completed check, input focus and success only, matching the mockup | Fix: secondary buttons were the same muted gray as disabled ones, disabled used a dashed border, and the accent survey CTA was a second competing "filled" style (the hierarchy read as weird) |
| 32 | Subtle depth pass | Depth instead of new loud elements: progress bar gets an inset track shadow + soft gradient/glow fill; `IconTile` gets a top-lit gradient + inner highlight; mission cards get a hairline `ring-brand-track/50`; `/reward` prize gets a soft accent halo and the cards a hairline ring. All low-contrast and on the existing palette | "Make the UI a tiny bit more interesting without stealing attention from the main points" |
